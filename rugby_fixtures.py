...
class Fixture:
    ...
    def __init__(self, time: datetime, home: str, away: str, tournament: str, time_fixed: bool = True):
        self.time = time
        self.home = home
        self.away = away
...
def get_local_fixtures() -> dict[str, Fixture]:
...
def update_saints_calendar():
    ...
    for match_id, match in fixture_list.items():
        ...
        if not (calendar_event := next((event for event in my_events
                                        if event.get('description', '') == match_id), None)):
            print('⭐')
